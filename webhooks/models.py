        ...
        for event_name, auto in HOOK_EVENTS.items():
            ...
            if action in model_config:
                raise ImproperlyConfigured(
                    "settings.HOOK_EVENTS have a duplicate {action} for model "
...
if getattr(settings, 'HOOK_THREADING', True):
    from webhooks.client import Client
    client = Client()
else:
...
class AbstractHook(models.Model):
    ...
    def serialize_hook(self, instance, updated_fields=None, payload_addition=None):
        ...
        if payload_addition is not None:
            data['extra'] = payload_addition
...
class Hook(AbstractHook):
    ...
    course = models.ForeignKey(Course, on_delete=models.CASCADE, related_name="webhooks")

    last_triggered_at = models.DateTimeField(null=True)
    last_triggered_status = models.CharField(max_length=255, null=True, blank=True)


    def save(self, *args, **kwargs):
        if self.pk:
            if self.last_triggered_status is not None:
...
def model_saved(sender, instance,
    ...
    if action == 'created':
        distill_model_event(instance, model_label, action, updated_fields=update_fields)
    elif update_fields is not None and len(update_fields) > 0:
        distill_model_event(instance, model_label, action, updated_fields=update_fields)
...
def custom_action(sender, action,
    ...
    payload_addition = kwargs.pop('payload_addition', None)
    model_label = get_model_label(instance)
    distill_model_event(instance, model_label, action, user_override=user,
                        updated_fields=updated_fields, payload_addition=payload_addition)
