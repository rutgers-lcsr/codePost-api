...
def safe_create_submissionfile_table(apps, schema_editor):
    ...
    Handles partial migration runs, corrupted table states, and orphaned tablespaces.
    """
    db_alias = schema_editor.connection.alias
    ...
    with schema_editor.connection.cursor() as cursor:
        ...
        try:
            ...
            if "doesn't exist" in error_str or "1932" in error_str or "1146" in error_str:
                print(f"ℹ Table 'core_submissionfile' is corrupted or doesn't exist")
                
                # Disable foreign key checks temporarily to force cleanup
                try:
                    ...
                    print(f"ℹ Drop attempt: {drop_err}")
                    pass
            else:
                raise
        
        # Create the table - use CREATE TABLE IF NOT EXISTS to handle edge cases
        print("ℹ Creating table 'core_submissionfile'")
        try:
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS `core_submissionfile` (
            ...
            # If we still get tablespace error, we need manual intervention
            if "184" in str(create_err) or "Tablespace already exists" in str(create_err):
                print(f"⚠ ERROR: Orphaned tablespace detected for core_submissionfile")
                print(f"   This requires manual cleanup. Please run on your database server:")
                print(f"   1. Stop MySQL/MariaDB")
                print(f"   2. Remove file: /var/lib/mysql/codepost/core_submissionfile.ibd")
                print(f"   3. Start MySQL/MariaDB")
                print(f"   4. Re-run migrations")
                raise
            else:
                raise
