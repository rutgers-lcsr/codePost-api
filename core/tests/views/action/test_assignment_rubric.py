...
class TestPermissions_Assignment_rubric_Base(BaseTestCases.TestPermissions):
...
class TestPermissions_Assignment_rubric(APITestCase):
  ...
  def test_permission_rubric_released(self):
    ...
    endpoint = reverse("assignment-rubric", args=[self.DB['Assignment'].id])
  ...
  def test_permission_rubric_not_released(self):
    student = Persona.STUDENT_OF_SUB(self)
    grader = Persona.GRADER_OF_SUB(self)
    other_grader = User.objects.get(username="grader_cos126_1@princeton.edu")
