...
if (TEST == 3):
  ...
  cos126s2019 = Course.objects.create(organization=princeton, period="S2019", name="COS126")
  cos126s2019.courseAdmins.add(user)
  cos126s2019.save()
...
if (TEST == 8):
  ...
  for i in range(1, 30):
    Comment.objects.create(text="good job! %s" % i, author=user, file=file, startChar=1, endChar=4, startLine=i, endLine=i)
  sub.isFinalized = False
  sub.grader = user
  sub.save()
...
if (TEST == 9):
  ...
  for i in range(0,12):
    ...
    Comment.objects.create(text="good job! File: %s" % i, author=user, file=file, startChar=4, endChar=10, startLine=1, endLine=1)
  sub.isFinalized = False
  sub.grader = user
  sub.save()
...
if (TEST == 10):
  ...
  file = File.objects.create(name="hello.java", code=long_code, submission=sub, extension='java')
  for i in range(1, 30):
    Comment.objects.create(text="good job! %s" % i, rubricComment=rubricComment, author=user, file=file, startChar=1, endChar=4, startLine=i, endLine=i)
  sub.isFinalized = False
  sub.grader = user
  sub.save()
...
if (TEST == 11):
  ...
  cos226s2019 = Course.objects.create(organization=princeton, period="S2019", name="COS226")
  cos226s2019.save()

  queues2019 = Assignment.objects.create(course=cos226s2019, points=20, isReleased=True, name="Hello")
  rubricCategory = RubricCategory.objects.create(assignment=queues2019,name='General',pointLimit=10)
  rubricComment = RubricComment.objects.create(text='Missing a semicolon', pointDelta=2, category=rubricCategory)

  username = 'student@student.io'
  student = User.objects.create(username=username, email=username, password="rootabega")
  student.profile.organization = princeton
  cos226s2019.students.add(student)
  cos226s2019.save()

  sub = Submission.objects.create(assignment=queues2019)
  sub.students.add(student)
  file = File.objects.create(name="hello.java", code=code, submission=sub, extension='java')
  sub.isFinalized = False
  sub.save()
